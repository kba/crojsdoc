...
(function() {
  ...
  _readConfig = function(options) {
    var config, crojsdoc_yaml, e, error, error1, k, package_json, plugins, ref1, safeLoad, v;
    config = {
      theme_opts: {}
    };
    try {
      package_json = JSON.parse(fs.readFileSync(join(process.cwd(), 'package.json'), 'utf-8'));
      ref1 = package_json.crojsdoc;
      for (k in ref1) {
        v = ref1[k];
        config[k] = v;
      }
  ...
  _parseArguments = function(options) {
    ...
    switches = [['-h', '--help', 'show help'], ['-o', '--output DIRECTORY', 'Output directory'], ['-t', '--title TITLE', 'Document Title'], ['-T', '--theme THEME', 'Theme to use (default: "default")'], ['-q', '--quiet', 'Less output'], ['-r', '--readme DIRECTORY', 'README.md directory path'], ['-f', '--files', 'Included source files'], ['--external-types JSONFILE', 'External type definitions']];
  ...
  exports.run = function() {
    var contents, e, error, options, result;
